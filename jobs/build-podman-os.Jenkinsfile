...
def generate_disk_images(artifacts, shortcommit, arch, repo) {
    generate_diskvar_json(shortcommit, arch, artifacts, repo)
...
def generate_diskvar_json(shortcommit, arch, artifacts, repo) {
...
lock(resource: "build-podman-os") {
    cosaPod(cpu: "${ncpus}",
            ...
            env: ["OSBUILD_EXPORT_FORCE_NO_PRESERVE_OWNER":"true"],
    ...
    try {
        ...
        withCredentials([file(credentialsId: 'ravanelli-push-registry-secret', variable: 'REGISTRY_SECRET')]) {
            stage("Push Manifest") {
                ...
                def manifest = "${params.CONTAINER_REGISTRY_STAGING_REPO}:5.0-${shortcommit}"
                def cmds = ""
                parallel archinfo.keySet().collectEntries{arch -> [arch, {
                    image = "docker://${params.CONTAINER_REGISTRY_STAGING_REPO}:${arch}-${shortcommit}"
                ...
                //  Supermin is unable to access the secret mounted in another directory.
                // Although this solution is not ideal, let's proceed with it until we
                // can find a more effective approach.
                // This doc includes a 'use' case that didn't function as expected.
                ...
                shwrap("""cosa supermin-run bash -c \
                    "/usr/bin/buildah manifest create ${manifest} ${cmds} && \
                    /usr/bin/buildah manifest push --authfile ./push-secret --all ${manifest} \
                    docker://${params.CONTAINER_REGISTRY_STAGING_REPO}:5.0-${shortcommit}"
                """)
                shwrap("""rm -Rf  push-secret""")
            }
        }
            ...
            withCredentials([file(credentialsId: 'ravanelli-push-registry-secret', variable: 'REGISTRY_SECRET')]) {
                stage("Release Manifest") {
                    shwrap("""
                        ...
                        docker://${params.CONTAINER_REGISTRY_STAGING_REPO}:5.0-${shortcommit} \
                        docker://${params.CONTAINER_REGISTRY_REPO}:5.0
                    """)
                }
            }
        }
